...
Quando injetamos o payload, ele é passado para o servidor que não possui filtros para mitigar XXE, sendo assim o servidor requisitar um parser e envia o output do XML parseado, que no nosso caso será o arquivo `bugs.txt` que será exposta ao atacante devido a query XML.
